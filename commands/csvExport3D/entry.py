...
CMD_ID = f'{config.COMPANY_NAME}_{config.ADDIN_NAME}_csvExport3D'
CMD_NAME = '3D Export (CSV)'
CMD_Description = 'Export 3D designs listed in a CSV file from a selected folder.'
IS_PROMOTED = True
...
def command_execute(args: adsk.core.CommandEventArgs):
    ...
    ui.messageBox(f'Initiating search for {len(csv_targets)} targets...\nProgress will be shown in the Text Command window.')
    ...
    if not matched_documents:
        ui.messageBox('No matching documents found in the selected folder.')
        return
    ...
    for i, doc_info in enumerate(matched_documents):
        ...
        try:
            ...
            if not design_prod:
                app.log('  Error: File is not a 3D design.')
                losses += 1
                continue
